...
//check for equality between current BST node and rhs BST node reference
template <typename T>
bool BST<T>::operator==(const BST<T>& rhs) const{
	return isSame(root_,rhs.root_);
}

//accepts two BST node pointers
template <typename T>
bool BST<T>::isSame(const typename BST<T>::Node* left, const typename BST<T>::Node* right) const{
	bool rc;
	//checks if left and right nodes are nullptr (empty)
	if(!left && !right){
		rc=true;
	}
	//if left and right are not nullptrs
	else if(left && right){
		//checks if data in left and right pointer are the same
		if(left->data_ == right->data_){
			//if they are, recursively call the function for each node's left and right children
...
int main(void){
	...
	BST<int> testTree[numTrees]; //root and sub-trees
	BST<int> correctTree[numTrees];
	BST<int> correctCopies[numTrees];
	int dataset[numNodes]={18,8,28,4,10,20,34,2,6,14,24,32,36,12,16,22,26,30,38}; //nodes for BST
	...
	int removeValues[numTrees]={18,18,18,2,32,36,10,28,4,8,18}; //values to remove from trees
