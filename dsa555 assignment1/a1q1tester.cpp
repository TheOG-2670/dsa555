...
int main(void) {
	TestPtr runTest[numTests] = { test1, 
...
*/
bool test1(std::string& error) {
	DisjointSet theSet(100); // sets max value for accepted range of new values added to sets
	bool result;
	bool rc = true;
	for (int i = 100; rc && i < 105; i++) { //makes sets
		result = theSet.makeSet(i); //creates new set for current value (should return false as values exceed max value)
		if (result == true) {
			rc = false;
