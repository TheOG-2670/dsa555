...
int partitionMedianOf3(int array[],int left, int right)
{
   ...
   else if (array[(left + right) / 2] > array[right])
   {
...
int partitionRandom(int array[],int left, int right)
{
   ...
   //array right as median
    int randomValue = rand() % ((right - left)+1) + left;
   
    //swap randomly selected value (as pivot) with right
   int pivot = array[randomValue];
   int temp = array[right];
   array[right] = pivot;
